...
export const getAttendanceLogs = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (employeeId) {
      const targetEmps = await Employee.find({ 
        employeeId: { $regex: employeeId as string, $options: 'i' } 
      }).select('_id');
    ...
    if (department) {
      const targetEmps = await Employee.find({ department: department as string }).select('_id');
      if (filter.employee) {
        // Intersect
        const existingIds = (filter.employee.$in as any[]).map(id => id.toString());
    ...
    if (startDate || endDate) {
      ...
      if (endDate) filter.date.$lte = endDate as string;
    }
...
export const getTodayUserStatus = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ message: 'User context not found' });
...
export const upsertManualRecord = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
